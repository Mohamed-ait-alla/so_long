...
/*   Updated: 2025/02/02 17:39:52 by mait-all         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static	void	ft_draw_ghost_animation(t_mlx_data *mlx, int i, int j)
{
	if (i == 59999)
		mlx_put_image_to_window(mlx->mlx_ptr, mlx->mlx_window,
			mlx->sprites.ghost_right[j % 6], mlx->ghosts[j].oldPosX * SIZE,
			mlx->ghosts[j].oldPosY * SIZE);
	else if (i == 69990)
		mlx_put_image_to_window(mlx->mlx_ptr, mlx->mlx_window,
			mlx->sprites.ghost_down[j % 6], mlx->ghosts[j].oldPosX * SIZE,
			mlx->ghosts[j].oldPosY * SIZE);
	else if (i == 89999)
		mlx_put_image_to_window(mlx->mlx_ptr, mlx->mlx_window,
			mlx->sprites.ghost_left[j % 6], mlx->ghosts[j].oldPosX * SIZE,
			mlx->ghosts[j].oldPosY * SIZE);
}
