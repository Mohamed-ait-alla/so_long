...
/*   Updated: 2025/02/01 10:37:36 by mait-all         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (abs((mlx->player_pos_x - mlx->ghosts[i].old_pos_x))
		> abs((mlx->player_pos_y - mlx->ghosts[i].old_pos_y)))
	{
		if ((mlx->player_pos_x - mlx->ghosts[i].old_pos_x) > 0
			&& mlx->map[mlx->ghosts[i].old_pos_y][mlx->ghosts[i].old_pos_x + 1]
			!= '1'
			)
			mlx->ghosts[i].n_pos_x++;
		else if ((mlx->player_pos_x - mlx->ghosts[i].old_pos_x) < 0
			&& mlx->map[mlx->ghosts[i].old_pos_y][mlx->ghosts[i].old_pos_x - 1]
			!= '1')
			mlx->ghosts[i].n_pos_x--;
	}
	...
	{
		if ((mlx->player_pos_y - mlx->ghosts[i].old_pos_y) > 0
			&& mlx->map[mlx->ghosts[i].old_pos_y + 1][mlx->ghosts[i].old_pos_x]
			!= '1')
			mlx->ghosts[i].n_pos_y++;
		else if ((mlx->player_pos_y - mlx->ghosts[i].old_pos_y) < 0
			&& mlx->map[mlx->ghosts[i].old_pos_y - 1][mlx->ghosts[i].old_pos_x]
			!= '1')
			mlx->ghosts[i].n_pos_y--;
	}
